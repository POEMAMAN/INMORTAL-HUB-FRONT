...
dotenv.config();
const CharacterHarryPotter = require('../../api/models/models.HarryPotter/characters.HarryPotter.model');

const arrayCharactersHarryPotter = [{
        name: "Harry Potter",
...
mongoose
    ...
    .then(async() => {
        ...
        if (allCharactersHarryPotter.length > 0) {
            await CharacterHarryPotter.collection.drop();
    ...
    .then(async() => {
        const charactersHarryPotterMap = arrayCharactersHarryPotter.map(
            (character) => new CharacterHarryPotter(character)
        );
        await CharacterHarryPotter.insertMany(charactersHarryPotterMap);
